...
public:
    ...
    */
    GenericPointer Append(SizeType index, Allocator* allocator = 0) const {
        char m_buffer[21];
        char* end = sizeof(SizeType) == 4 ? internal::u32toa(index, m_buffer) : internal::u64toa(index, m_buffer);
        SizeType length = static_cast<SizeType>(end - m_buffer);
        m_buffer[length] = '\0';

        if (sizeof(Ch) == 1) {
            Token token = { reinterpret_cast<Ch*>(m_buffer), length, index };
            return Append(token, allocator);
        }
        else {
            Ch name[21];
            for (size_t i = 0; i <= length; i++)
                name[i] = static_cast<Ch>(m_buffer[i]);
