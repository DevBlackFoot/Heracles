        ...
        if (const ValueType* v = GetMember(value, GetEnumString())) {
            if (v->IsArray() && v->Size() > 0) {
                ...
                for (ConstValueIterator itr = v->Begin(); itr != v->End(); ++itr) {
                    ...
                    char m_buffer[256u + 24];
                    MemoryPoolAllocator<> hasherAllocator(m_buffer, sizeof(m_buffer));
...
struct TokenHelper {
    RAPIDJSON_FORCEINLINE static void AppendIndexToken(Stack& documentStack, SizeType index) {
        *documentStack.template Push<Ch>() = '/';
        char m_buffer[21];
        size_t length = static_cast<size_t>((sizeof(SizeType) == 4 ? u32toa(index, m_buffer) : u64toa(index, m_buffer)) - m_buffer);
        for (size_t i = 0; i < length; i++)
            *documentStack.template Push<Ch>() = static_cast<Ch>(m_buffer[i]);
    }
};
...
struct TokenHelper<Stack, char> {
    RAPIDJSON_FORCEINLINE static void AppendIndexToken(Stack& documentStack, SizeType index) {
        if (sizeof(SizeType) == 4) {
            char *m_buffer = documentStack.template Push<char>(1 + 10); // '/' + uint
            *m_buffer++ = '/';
            const char* end = internal::u32toa(index, m_buffer);
             documentStack.template Pop<char>(static_cast<size_t>(10 - (end - m_buffer)));
        }
        else {
            char *m_buffer = documentStack.template Push<char>(1 + 20); // '/' + uint64
            *m_buffer++ = '/';
            const char* end = internal::u64toa(index, m_buffer);
            documentStack.template Pop<char>(static_cast<size_t>(20 - (end - m_buffer)));
        }
    }
};
