...
namespace AlmondDxGraphics.GraphicResources.Factories
{
    public static class TextureFactory
    {
        ...
        private static ShaderResourceView InitTextureFromData(Device11 d3dDevice, DeviceContext context, DDS_HEADER header, DDS_HEADER_DXT10? header10, byte[] bitData, int offset, int maxsize, out bool isCubeMap)
        {
        ...
        private static ShaderResourceView CreateTextureFromDDS(Device11 device, DeviceContext context, byte[] data, out bool isCubeMap)
        {
        ...
        private static ShaderResourceView CreateTextureFromBitmap(Device11 device, DeviceContext context, string filename)
        {
        ...
        private static ShaderResourceView CreateTextureFromWIC(Device11 device, DeviceContext context, string filename)
        {
            // 로드할때마다 팩토리 생성? 좀 그렇긴 한데.. 일단 하고 나중에 리소스화시키자.
            var wicFactory = new ImagingFactory2();
            var bitmapDecoder = new BitmapDecoder(wicFactory, filename, DecodeOptions.CacheOnDemand);
            ...
            var resourceView = new ShaderResourceView(device, texture2D);
            texture2D.Dispose();
            return resourceView;
        }
        ...
        public static ShaderResourceView LoadTextureFromFile(DxDevice device, string filename)
        {
            ...
            switch (ext.ToLower())
            {
                case ".dds":
                    {
                        bool isCube;
                        return CreateTextureFromDDS(device.Device, device.DeviceContext,
                            System.IO.File.ReadAllBytes(filepath), out isCube);                        
                    }                    
                case ".bmp":
                    {
                        return CreateTextureFromBitmap(device.Device, device.DeviceContext, filepath);                        
                    }                    
                default:
                    {
                        return CreateTextureFromWIC(device.Device, device.DeviceContext, filepath);
                    }
            }
        }
    }
